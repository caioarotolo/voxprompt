...
| `VOXPROMPT_TEMPLATE` | `spec` | template inicial (`spec`/`prompt`/`formal`/`whatsapp`/`conversa`/`reuniao`/`raw`) |
...
| `t` | cicla template `spec → prompt → formal → whatsapp → conversa → reuniao → raw` |
